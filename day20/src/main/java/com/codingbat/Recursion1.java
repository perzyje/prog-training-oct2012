...
final class Recursion1 {
	...
	public int triangle(int rows) {
		...
		return triangle(rows - 1) + triangleLine(rows);
	}

	private int triangleLine(int row) {
		if (row <= 0) {
			return 0;
		}
		if (row == 1) {
	...
	public int countX(String str) {
		...
		return str.charAt(0) == 'x' ? 1 + countX(str.substring(1)) : countX(str
				.substring(1));
	}
	...
	public int array11(int[] nums, int index) {
		...
		return nums[index] == 11 ? 1 + array11(nums, index + 1) : array11(nums,
				index + 1);
	}
	...
	public int countAbc(String str) {
		...
		return (str.startsWith("abc") || str.startsWith("aba")) ? 1 + countAbc(str
				.substring(1)) : countAbc(str.substring(1));
	}
	...
	public String parenBit(String str) {
		int start = str.indexOf("("), end = str.indexOf(")", start);
